...
file1= csv.reader(open('C:/Users/wasay/Desktop/DSC 540/Final/dataNoCurricular.csv'), delimiter=',', quotechar='"')
...
if binning==1 and cross_val==1:
    ...
    sns.lineplot(x=fpr, y=tpr, color='green', lw=2, label=f'ROC Curve (AUC = {roc_auc:.2f})')
    sns.lineplot(x=[0, 1], y=[0, 1], color='gray', linestyle='--', label="Random Classifier")
    ...
    start_ts=time.time()
    clf= AdaBoostClassifier(n_estimators = 100, estimator = None, learning_rate = 0.1, random_state = rand_st)
    scores= cross_validate(estimator = clf, X = data_np, y = target_np, scoring = scorers, cv = 5)

...
file1= csv.reader(open('C:/Users/wasay/Desktop/DSC 540/Final/featureEngineering.csv'), delimiter=',', quotechar='"')
...
if feat_select==1:
    ...
    if fs_type==2:
        clf = RandomForestClassifier(n_estimators = 20, criterion = "entropy", max_depth = None, min_samples_split = 3, random_state = rand_st)              
    ...
    if fs_type==4:
        ...
        for x in clf.feature_importances_: 
            if x >= np.mean(clf.feature_importances_):
                sel_idx.append(1)  
            elif x < np.mean(clf.feature_importances_):
                sel_idx.append(0)
...
if cross_val==0:    
    ...
    clf = RandomForestClassifier(n_estimators = 50, criterion = "entropy", max_depth = None, min_samples_split = 3, random_state = rand_st)   
